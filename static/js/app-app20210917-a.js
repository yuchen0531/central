webpackJsonp([1],{

/***/ "4gZJ":
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__.p + "static/img/picture_5_podcast.7669228.jpg";

/***/ }),

/***/ "9WAJ":
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ "NHnr":
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });

// EXTERNAL MODULE: ./node_modules/vue/dist/vue.esm.js
var vue_esm = __webpack_require__("7+uW");

// CONCATENATED MODULE: ./node_modules/babel-loader/lib!./node_modules/vue-loader/lib/selector.js?type=script&index=0!./src/App.vue
//
//
//
//
//
//
//

/* harmony default export */ var App = ({
  name: 'App'
});
// CONCATENATED MODULE: ./node_modules/vue-loader/lib/template-compiler?{"id":"data-v-0b02ba34","hasScoped":false,"transformToRequire":{"video":["src","poster"],"source":"src","img":"src","image":"xlink:href"},"buble":{"transforms":{}}}!./node_modules/vue-loader/lib/selector.js?type=template&index=0!./src/App.vue
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{attrs:{"id":"app"}},[_c('router-view')],1)}
var staticRenderFns = []
var esExports = { render: render, staticRenderFns: staticRenderFns }
/* harmony default export */ var selectortype_template_index_0_src_App = (esExports);
// CONCATENATED MODULE: ./src/App.vue
function injectStyle (ssrContext) {
  __webpack_require__("WCFv")
}
var normalizeComponent = __webpack_require__("VU/8")
/* script */


/* template */

/* template functional */
var __vue_template_functional__ = false
/* styles */
var __vue_styles__ = injectStyle
/* scopeId */
var __vue_scopeId__ = null
/* moduleIdentifier (server only) */
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  App,
  selectortype_template_index_0_src_App,
  __vue_template_functional__,
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
)

/* harmony default export */ var src_App = (Component.exports);

// EXTERNAL MODULE: ./node_modules/babel-runtime/core-js/object/keys.js
var keys = __webpack_require__("fZjL");
var keys_default = /*#__PURE__*/__webpack_require__.n(keys);

// EXTERNAL MODULE: ./node_modules/babel-runtime/core-js/array/from.js
var array_from = __webpack_require__("c/Tr");
var from_default = /*#__PURE__*/__webpack_require__.n(array_from);

// EXTERNAL MODULE: ./node_modules/vue-router/dist/vue-router.esm.js
var vue_router_esm = __webpack_require__("/ocq");

// EXTERNAL MODULE: ./node_modules/vue-meta/dist/vue-meta.esm.js
var vue_meta_esm = __webpack_require__("f3bp");

// CONCATENATED MODULE: ./src/data/AllData.js
var dataList = [{
  title: '永續美妝保養品消費調查',
  img: __webpack_require__("j/ML"),
  isHot: true,
  banner: __webpack_require__("nbZn"),
  pagecount: 66,
  date: '2021/10/7',
  url: 'https://forms.gle/Ej6iZNPdUhMeW816A',
  filetype: 'PDF',
  purchaseWay: 'qqDownload fron this page',
  price: '$24.02',
  main: '1111Smartphones, mobile phones with more advanced computing capabilities and connectivity than regular mobile phones, came onto the consumer market in the late 90s, but only gained mainstream popularity with the introduction of Apple’s iPhone in 2007. The iPhone revolutionized the industry by offering customer friendly features such as a touch screen interface and a virtual keyboard. The first smartphone running on Android was introduced to the consumer market in late 2008.',
  abstract: '新冠疫情改變了人們的生活方式，也讓人們對環保議題的關注隨之提升，相對的也讓重視永續、環保的永續美妝保養品日益受到重視。本研究針對一般消費者對於美妝保養品的消費行為、資訊蒐集管道、選擇因素等進行調查外，更從消費者永續態度調查、對市售永續美妝保養品牌評價、消費通路、未來購買意願等方向進行深入分析。<br>研究發現，近七成的台灣消費者認同永續美妝保養品的理念，甚至願意較一般美妝保養品多付三成的價格購買符合永續標準的美妝保養品，沒有購買永續美妝保養品的主要原因是沒有足夠的資訊、或是定價高過其接受度。也因此在普遍偏好購買進口商品的美妝保養市場中，讓本土永續美妝保養品有了立足成長的空間。<br>本研究提供相關的調查與數據，相信能為美妝保養品的品牌商、通路商提供不同的思考面向，提供商業規劃、策略提案的參考。<br>納入調查分析的次品類有：<br>永續皮膚護理產品、永續頭髮護理產品、永續彩妝產品、永續防曬產品、永續香氛產品、永續指甲護理產品',
  include: [],
  maincatalogue: [{ title: '1. 導言',
    pages: '9',
    content: [{ title: '1.1 產業背景', pages: '9' }, { title: '1.2 研究目標', pages: '10' }]
  }, { title: '2. 資料收集方法與人口統計',
    pages: '11',
    content: [{ title: '2.1 資料收集與信效度分析', pages: '11' }, { title: '2.2 人口統計', pages: '12' }]
  }, { title: '3. 一般美妝保養品消費行為',
    pages: '14',
    content: [{ title: '3.1 美妝保養品消費金額', pages: '14' }, { title: '3.2 美妝保養品消費國內品牌比例', pages: '17' }, { title: '3.3 美妝保養品消費情境', pages: '18' }, { title: '3.4 美妝保養品資訊蒐集管道', pages: '20' }, { title: '3.5 美妝保養品選擇因素', pages: '25' }]
  }, { title: '4. 永續態度調查',
    pages: '29',
    content: [{ title: '4.1 永續態度認同程度', pages: '29' }, { title: '4.2 七項永續美妝保養品特性的認同程度 ', pages: '34' }]
  }, { title: '5. 永續美妝保養品品牌評價',
    pages: '36',
    content: [{ title: '5.1 永續美妝保養品品牌評價', pages: '36' }, { title: '5.2 品牌知名度', pages: '42' }]
  }, { title: '6. 永續美妝保養品消費比例及通路分析',
    pages: '47',
    content: [{ title: '6.1 永續美妝保養品消費行為', pages: '47' }, { title: '6.2 永續美妝保養品消費者購買類型', pages: '50' }, { title: '6.3 永續皮膚護理產品', pages: '52' }, { title: '6.4 永續頭髮護理產品', pages: '56' }, { title: '6.5 永續彩妝產品', pages: '59' }, { title: '6.6 永續防曬產品', pages: '64' }, { title: '6.7 永續香氛產品', pages: '69' }, { title: '6.8 永續指甲護理產品', pages: '72' }]
  }, { title: '7. 未來購買意願',
    pages: '78',
    content: [{ title: '7.1 消費者未來購買永續美妝保養品態度', pages: '78' }, { title: '7.2 消費者未來有意願購買的類型', pages: '79' }, { title: '7.3 消費者未來願付價格', pages: '82' }]
  }, { title: '8 結論', pages: '86' }],
  picturecatalogue: [{ title: '圖 1、2020 年美妝保養品消費金額 ', pages: '14' }, { title: '圖 2、男女消費金額比例', pages: '15' }, { title: '圖 3、消費在國內品牌的比例', pages: '18' }, { title: '圖 4、在哪些情況購買美妝保養產品（複選）', pages: '19' }, { title: '圖 5、最常在哪個促銷檔期購買美妝保養產品', pages: '19' }, { title: '圖 6、最常在哪裡看到美妝保養產品的廣告', pages: '20' }, { title: '圖 7、美妝保養資訊搜尋管道（複選）', pages: '21' }, { title: '圖 8、線上美妝保養資訊管道搜尋趨勢', pages: '22' }, { title: '圖 9、最常觀看美妝保養品前五大頻道', pages: '23' }, { title: '圖 10、最常觀看美妝保養品前五大粉絲專頁', pages: '23' }, { title: '圖 11、最常觀看美妝保養的網站', pages: '24' }, { title: '圖 12、民眾對於美妝保養品的選擇因素（複選）', pages: '25' }, { title: '圖 13、不同性別民眾對於美妝保養品的選擇因素（複選）', pages: '26' }, { title: '圖 14、民眾對於永續產品利他動機的同意程度', pages: '30' }, { title: '圖 15、民眾對於永續產品態度的同意程度', pages: '31' }, { title: '圖 16、民眾對於永續產品主觀規範的同意程度', pages: '32' }, { title: '圖 17、民眾對於永續產品社群媒體影響的同意程度', pages: '33' }, { title: '圖 18、七項永續特性民眾認知重要性', pages: '35' }, { title: '圖 19、「成分選擇上」做最好的品牌', pages: '36' }, { title: '圖 20、「成分有機」做最好的品牌', pages: '37' }, { title: '圖 21、「產品包裝可回收、重複使用」做最好的品牌', pages: '37' }, { title: '圖 22、「製程再生能源使用、製程水資源管理」做最好的品牌', pages: '38' }, { title: '圖 23、「保障人權公平貿易」做最好的品牌', pages: '38' }, { title: '圖 24、七項永續特性民眾重視的比例', pages: '39' }, { title: '圖 25、七項永續特性不同性別民眾重視的比例', pages: '40' }, { title: '圖 26、七項永續特性不同年齡民眾重視的比例', pages: '40' }, { title: '圖 27、永續美妝保養品牌知名度（複選）', pages: '42' }, { title: '圖 28、不同性別間的永續品牌知名度', pages: '43' }, { title: '圖 29、不同年齡間的永續品牌知名度', pages: '43' }, { title: '圖 30、永續美妝保養品牌消費比例（複選）', pages: '44' }, { title: '圖 31、不同性別間永續美妝保養品牌消費比例（複選）', pages: '45' }, { title: '圖 32、不同年齡間永續美妝保養品牌消費比例（複選）', pages: '45' }, { title: '圖 33、2020 年消費永續美妝保養品比例', pages: '47' }, { title: '圖 34、不同性別之2020 年永續美妝保養品消費比例', pages: '48' }, { title: '圖 35、不願意購買永續美妝保養品的原因', pages: '50' }, { title: '圖 36、購買過以下哪些類型的永續美妝保養產品（複選）', pages: '51' }, { title: '圖 37、各地區購買永續美妝保養品類型', pages: '51' }, { title: '圖 38、選擇「永續皮膚護理產品」的原因（複選）', pages: '52' }, { title: '圖 39、購買「永續皮膚護理產品」願意較一般皮膚護理產品多付幾成', pages: '53' }, { title: '圖 40、在哪些通路購買「永續皮膚護理產品」（複選）', pages: '54' }, { title: '圖 41、「永續皮膚護理產品」最常購買的量販超市通路（複選）', pages: '54' }, { title: '圖 42、「永續皮膚護理產品」最常購買的藥妝通路（複選）', pages: '55' }, { title: '圖 43、「永續皮膚護理產品」最常購買的電商平台通路（複選）', pages: '55' }, { title: '圖 44、選擇「永續頭髮護理產品」的原因（複選）', pages: '56' }, { title: '圖 45、購買「永續頭髮護理產品」願意較一般頭髮護理產品多付幾成', pages: '57' }, { title: '圖 46、在哪些通路購買「永續頭髮護理產品」（複選）', pages: '57' }, { title: '圖 47、「永續頭髮護理產品」最常購買的量販超市通路（複選）', pages: '58' }, { title: '圖 48、「永續頭髮護理產品」最常購買的藥妝通路（複選）', pages: '58' }, { title: '圖 49、「永續頭髮護理產品」最常購買的電商平台通路（複選）', pages: '59' }, { title: '圖 50、選擇「永續彩妝產品」的原因（複選）', pages: '60' }, { title: '圖 51、購買「永續彩妝產品」願意較一般彩妝產品多付幾成', pages: '61' }, { title: '圖 52、在哪些通路購買「永續彩妝產品」（複選）', pages: '61' }, { title: '圖 53、「永續彩妝產品」最常購買的量販超市通路（複選）', pages: '62' }, { title: '圖 54、「永續彩妝產品」最常購買的藥妝通路（複選）', pages: '63' }, { title: '圖 55、「永續彩妝產品」最常購買的電商平台通路（複選）', pages: '63' }, { title: '圖 56、選擇「永續防曬產品」的原因（複選）', pages: '64' }, { title: '圖 57、購買「永續防曬產品」願意較一般防曬產品多付幾成', pages: '65' }, { title: '圖 58、在哪些通路購買「永續防曬產品」（複選）', pages: '65' }, { title: '圖 59、「永續防曬產品」最常購買的量販超市通路（複選）', pages: '66' }, { title: '圖 60、「永續防曬產品」最常購買的藥妝通路（複選）', pages: '67' }, { title: '圖 61、「永續防曬產品」最常購買的電商平台通路（複選）', pages: '67' }, { title: '圖 62、選擇「永續香氛產品」的原因（複選）', pages: '69' }, { title: '圖 63、購買「永續香氛產品」願意較一般香氛產品多付幾成', pages: '70' }, { title: '圖 64、在哪些通路購買「永續香氛產品」（複選）', pages: '70' }, { title: '圖 65、「永續香氛產品」最常購買的量販超市通路（複選）', pages: '71' }, { title: '圖 66、「永續香氛產品」最常購買的藥妝通路（複選）', pages: '71' }, { title: '圖 67、「永續香氛產品」最常購買的電商平台通路（複選）', pages: '72' }, { title: '圖 68、選擇「永續指甲護理產品」的原因（複選）', pages: '73' }, { title: '圖 69、購買「永續指甲護理產品」願意較一般指甲護理產品多付幾成', pages: '74' }, { title: '圖 70、在哪些通路購買「永續指甲護理產品」（複選）', pages: '74' }, { title: '圖 71、「永續指甲護理產品」最常購買的量販超市通路（複選）', pages: '75' }, { title: '圖 72、「永續指甲護理產品」最常購買的藥妝通路（複選）', pages: '76' }, { title: '圖 73、「永續指甲護理產品」最常購買的電商平台通路（複選）', pages: '76' }, { title: '圖 74、民眾未來購買態度', pages: '79' }, { title: '圖 75、未來願意購買什麼類型的永續美妝保養品', pages: '80' }, { title: '圖 76、不同性別未來願意購買的永續美妝保養品類型', pages: '81' }, { title: '圖 77、是否願為了永續美妝保養品多付一些錢', pages: '83' }, { title: '圖 78、各年齡間是否願為了永續美妝保養品多付一些錢', pages: '84' }, { title: '圖 79、未來願意多付幾成的金額購買永續美妝保養品', pages: '85' }],
  tablecatalogue: [{ title: '表 1、樣本變數資訊', pages: '11' }, { title: '表 2、正式問卷信效度表', pages: '11' }, { title: '表 3、人口統計辨識的樣本分布', pages: '12' }, { title: '表 4：所得變數的樣本分布', pages: '13' }, { title: '表 5、各年齡消費金額分布', pages: '16' }, { title: '表 6、各所得區間消費金額分布', pages: '17' }, { title: '表 7、不同年齡民眾對於美妝保養品的選擇因素（複選）', pages: '27' }, { title: '表 8、不同所得民眾對於美妝保養品的選擇因素（複選）', pages: '28' }, { title: '表 9：不同年齡之2020 年永續美妝保養品消費比例', pages: '49' }, { title: '表 10、不同所得區間之2020 年永續美妝保養品消費比例', pages: '49' }, { title: '表 11、不同年齡層未來願意購買的永續美妝保養類型', pages: '82' }],
  seoshare: {
    title: 'UUPON 永續美妝保養品消費調查',
    description: '2021市場調查研究：美妝保養品消費行為、環保永續美妝保養品牌評價、永續美妝保養品願意支付金額。'
  }
}, {
  title: '串流影音及Podcast 使用與消費行為調查',
  img: __webpack_require__("4gZJ"),
  isHot: true,
  banner: __webpack_require__("maJG"),
  pagecount: 135,
  date: '2021/10/7',
  url: 'https://forms.gle/Ej6iZNPdUhMeW816A',
  filetype: 'PDF',
  purchaseWay: 'wwDownload fron this page',
  price: '$104.02',
  main: '2222Smartphones, mobile phones with more advanced computing capabilities and connectivity than regular mobile phones, came onto the consumer market in the late 90s, but only gained mainstream popularity with the introduction of Apple’s iPhone in 2007. The iPhone revolutionized the industry by offering customer friendly features such as a touch screen interface and a virtual keyboard. The first smartphone running on Android was introduced to the consumer market in lte 2008.',
  abstract: '串流影音、Podcast是近5年來成長最快速的娛樂媒體市場之一，而在新冠疫情影響下，更加速兩個使用習慣的增長。本研究在2021年第二季進行市調研究，分別探討千禧世代與Z世代族群，分別在使用串流影音、Podcast上習慣與偏好的不同，也完整將主要使用平台納入調查，瞭解二族群閱聽眾在不同平台的上的使用行為、偏好異同，做為媒體平台計劃、媒體購買策略擬定的參考。<br>納入串流影音調查的平台有：<br>Netflix, Line TV, Youtube Premium, 愛奇藝, KKTV, Friday影音, 中華電信Hami Video, Disney+, Amazon Prime, Apple iTune電影, HBO M, Fox+, LiTV線上影視, 台灣大My Video, 公視+, 巴哈姆特動畫瘋, 嗶哩哩哩bilibili, 優酷影視<br>納入Podcast調查的平台有：<br>Youtube, Google Podcast, KKBOX, Spotify, Apple Podcast, Firstory, Himalaya, Pocket Casts, Overcast, Castbox',
  include: [],
  maincatalogue: [{ title: '1. 導言',
    pages: '11',
    content: [{ title: '1.1 產業背景', pages: '11' }, { title: '1.2 報告目標', pages: '12' }]
  }, { title: '2. 資料收集方法與人口統計',
    pages: '13',
    content: [{ title: '2.1 資料收集方法與信效度', pages: '13' }, { title: '2.2 人口統計', pages: '15' }]
  }, { title: '3. 千禧世代之串流影音使用行為態度分析',
    pages: '22',
    content: [{ title: '3.1 觀看的串流影音類型', pages: '22' }, { title: '3.2 觀看串流影音的類型及時長', pages: '23' }, { title: '3.3 喜愛串流影音內容的原因', pages: '25' }, { title: '3.4 對串流影音的態度 ', pages: '27' }, { title: '3.5 對串流影音平台偏好度排名', pages: '32' }, { title: '3.6 對串流影音廣告接受度', pages: '39' }, { title: '3.7 受訪者有付費訂閱哪些串流影音', pages: '42' }, { title: '3.8 受訪者取消付費的串流影音', pages: '44' }, { title: '3.9 受訪者新增付費的平台', pages: '46' }]
  }, { title: '4. 千禧世代聆聽PODCAST 平台',
    pages: '50',
    content: [{ title: '4.1 聆聽的PODCAST 類型', pages: '50' }, { title: '4.2 何時聆聽PODCAST', pages: '53' }, { title: '4.3 聆聽PODCAST 的類型及時長', pages: '53' }, { title: '4.4 對PODCAST 的態度', pages: '56' }, { title: '4.5 PODCAST 平台偏好度排名', pages: '60' }, { title: '4.6 對PODCAST 廣告接受度', pages: '69' }, { title: '4.7 未來訂閱取向', pages: '71' }]
  }, { title: '5. Z 世代之串流影音使用行為態度分析',
    pages: '74',
    content: [{ title: '5.1 觀看的串流影音類型', pages: '74' }, { title: '5.2 觀看串流影音的類型及時長', pages: '75' }, { title: '5.3 喜愛串流影音內容的原因', pages: '78' }, { title: '5.4 對串流影音的態度', pages: '80' }, { title: '5.5 對串流影音偏好度排名', pages: '85' }, { title: '5.6 廣告接受度', pages: '92' }, { title: '5.7 受訪者付費訂閱哪些串流影音', pages: '95' }, { title: '5.8 受訪者取消付費的串流影音', pages: '98' }, { title: '5.9 受訪者新增付費的平台', pages: '100' }]
  }, { title: '6. Z 世代 聆聽PODCAST 平台',
    pages: '103',
    content: [{ title: '6.1 PODCAST 訂閱選擇', pages: '103' }, { title: '6.2 何時聆聽', pages: '105' }, { title: '6.3 聆聽PODCAST 的類型及時長 ', pages: '107' }, { title: '6.4 對PODCAST 的態度', pages: '108' }, { title: '6.5 對平台內容的偏好度', pages: '113' }, { title: '6.6 廣告接受度', pages: '122' }, { title: '6.7 未來訂閱取向', pages: '124' }]
  }, { title: '7.結論',
    pages: '127'
  }],
  picturecatalogue: [{ title: '圖 1、本調查性別分布-千禧世代', pages: '15' }, { title: '圖 2、本調查居住地分布-千禧世代', pages: '16' }, { title: '圖 3、本調查個人薪資所得比例分布-千禧世代', pages: '17' }, { title: '圖 4、本調查家庭所得年收入比例分布-千禧世代', pages: '18' }, { title: '圖 5、本調查性別分布-Z 世代', pages: '18' }, { title: '圖 6、本調查居住地分布-Z 世代', pages: '19' }, { title: '圖 7、本調查個人薪資所得比例分布-Z 世代', pages: '20' }, { title: '圖 8、本調查家庭所得年收入比例分布-Z 世代', pages: '21' }, { title: '圖 9、每週平均觀看一個小時以上的串流影音平台', pages: '22' }, { title: '圖 10、串流影音平台每週平均觀看時長', pages: '23' }, { title: '圖 11、最常觀看哪種內容類型的串流影音', pages: '24' }, { title: '圖 12、根據最常看的內容平均每週觀看時長', pages: '24' }, { title: '圖 13、會觀看此內容類型的原因', pages: '25' }, { title: '圖 14、發現新的串流影音內容影響最大的因素', pages: '26' }, { title: '圖 15、如果不知道想看什麼內容，一開始會採用的方式', pages: '27' }, { title: '圖 16、對串流影音喜愛程度', pages: '28' }, { title: '圖 17、對串流影音的感知價值', pages: '29' }, { title: '圖 18、串流影音的知覺易用性', pages: '30' }, { title: '圖 19、串流影音的便利價值', pages: '31' }, { title: '圖 20、串流影音的行為意圖', pages: '32' }, { title: '圖 21、最容易上手的串流影音', pages: '33' }, { title: '圖 22、搜尋內容最容易的串流影音', pages: '34' }, { title: '圖 23、最讓人信賴的串流影音', pages: '35' }, { title: '圖 24、內容最完整的串流影音', pages: '36' }, { title: '圖 25、最令人滿意度的串流影音', pages: '37' }, { title: '圖 26、比較容易找到要看的串流影音內容', pages: '38' }, { title: '圖 27、千禧世代對於選擇影音內容的想法', pages: '39' }, { title: '圖 28、千禧世代預計要花多少分鐘才能找到要觀看的內容', pages: '39' }, { title: '圖 29、若大幅降低訂閱成本，願意透過串流影音觀看廣告', pages: '40' }, { title: '圖 30、願意觀看具有置入性廣告的串流影音', pages: '41' }, { title: '圖 31、看完廣告後採取的反應及行動', pages: '41' }, { title: '圖 32、有付費訂閱串流影音平台', pages: '42' }, { title: '圖 33、有付費訂閱的平台', pages: '43' }, { title: '圖 34、是否願意續訂付費串流影音平台', pages: '43' }, { title: '圖 35、會續訂的付費串流影音平台', pages: '44' }, { title: '圖 36、會取消付費的串流影音平台', pages: '45' }, { title: '圖 37、取消訂閱付費串流影音的原因', pages: '45' }, { title: '圖 38、改變主意繼續訂閱串流影音的原因', pages: '46' }, { title: '圖 39、願意新增訂閱付費串流影音平台', pages: '47' }, { title: '圖 40、願意付費訂閱新的串流影音平台', pages: '47' }, { title: '圖 41、付費訂閱串流影音平台具備最重要的三個功能或特色', pages: '48' }, { title: '圖 42、觀看串流影音平台具備最重要的三個功能或特色', pages: '49' }, { title: '圖 43、每週平均聆聽一個小時以上Podcast 平台', pages: '50' }, { title: '圖 44、平均每週聆聽Podcast 的總時數', pages: '51' }, { title: '圖 45、聆聽Podcast 須具備最重要的三個功能或特色', pages: '52' }, { title: '圖 46、訂閱的Podcast 節目數', pages: '52' }, { title: '圖 47、通常在哪種情況下聆聽Podcast', pages: '53' }, { title: '圖 48、最常聆聽的Podcast 類型', pages: '54' }, { title: '圖 49、在最常聆聽的Podcast 類型，每週聆聽時數', pages: '55' }, { title: '圖 50、聆聽特定Podcast 節目原因', pages: '55' }, { title: '圖 51、發現新Podcast 的地方', pages: '56' }, { title: '圖 52、對Podcast 喜愛程度', pages: '56' }, { title: '圖 53、對Podcast 的感知價值', pages: '57' }, { title: '圖 54、Podcast 的知覺易用性', pages: '58' }, { title: '圖 55、Podcast 的知覺有用性', pages: '59' }, { title: '圖 56、Podcast 的行為意圖', pages: '60' }, { title: '圖 57、最容易上手的Podcast', pages: '61' }, { title: '圖 58、搜尋內容最容易的Podcast', pages: '62' }, { title: '圖 59、最讓人信賴的Podcast', pages: '63' }, { title: '圖 60、內容最完整的Podcast', pages: '64' }, { title: '圖 61、最令人滿意度的Podcast', pages: '65' }, { title: '圖 62、比較容易找到要聽的Podcast 內容', pages: '66' }, { title: '圖 63、千禧世代對於選擇Podcast 的想法', pages: '67' }, { title: '圖 64、千禧世代預計要花多少分鐘才能找到要聆聽的內容', pages: '68' }, { title: '圖 65、願意聽具有置入性廣告的Podcast', pages: '69' }, { title: '圖 66、今年一月到三月曾經購買Podcast 上廣告所推銷的商品', pages: '70' }, { title: '圖 67、聽完Podcast 廣告後，會採取何種反應及行動', pages: '70' }, { title: '圖 68、未來訂閱取向-沒有意願聆聽Podcast 的原因', pages: '71' }, { title: '圖 69、未來有意願聆聽Podcast 的類型', pages: '72' }, { title: '圖 70、沒有意願聆聽Podcast 的原因', pages: '73' }, { title: '圖 71、每週平均觀看一個小時以上的串流影音平台', pages: '74' }, { title: '圖 72、每週觀看串流影音平台的總時數', pages: '75' }, { title: '圖 73、最常觀看哪種內容類型的串流影音', pages: '76' }, { title: '圖 74、根據最常看的內容平均每週觀看時長', pages: '77' }, { title: '圖 75、會觀看此內容類型的原因', pages: '78' }, { title: '圖 76、發現新的串流影音內容影響最大的因素', pages: '79' }, { title: '圖 77、如果不知道想看什麼內容，一開始會採用的方式', pages: '80' }, { title: '圖 78、對串流影音喜愛程度', pages: '81' }, { title: '圖 79、對串流影音的感知價值', pages: '82' }, { title: '圖 80、串流影音的知覺易用性', pages: '83' }, { title: '圖 81、串流影音的便利價值', pages: '84' }, { title: '圖 82、串流影音的行為意圖', pages: '85' }, { title: '圖 83、最容易上手的串流影音', pages: '86' }, { title: '圖 84、搜尋內容最容易的串流影音', pages: '87' }, { title: '圖 85、讓人信賴的串流影音', pages: '88' }, { title: '圖 86、內容最完整的串流影音', pages: '89' }, { title: '圖 87、最令人滿意度的串流影音', pages: '90' }, { title: '圖 88、比較容易找到要看的串流影音內容', pages: '91' }, { title: '圖 89、Z 世代對於選擇影音內容的想法', pages: '91' }, { title: '圖 90、Z 世代預計要花多少分鐘才能找到要觀看的內容', pages: '92' }, { title: '圖 91、若大幅降低訂閱成本，願意透過串流影音觀看廣告', pages: '93' }, { title: '圖 92、願意觀看具有置入性廣告的串流影音', pages: '94' }, { title: '圖 93、看完廣告後採取的反應及行動', pages: '94' }, { title: '圖 94、有付費訂閱串流影音平台', pages: '95' }, { title: '圖 95、有付費訂閱的平台', pages: '96' }, { title: '圖 96、願意續訂付費串流影音平台', pages: '97' }, { title: '圖 97、會續訂的付費串流影音平台', pages: '98' }, { title: '圖 98、會取消付費串流影音平台', pages: '99' }, { title: '圖 99、取消訂閱付費串流影音的原因', pages: '99' }, { title: '圖 100、願意新增訂閱付費串流影音平台', pages: '100' }, { title: '圖 101、願意新增訂閱付費串流影音平台(複選)', pages: '101' }, { title: '圖 102、付費訂閱串流影音平台具備最重要的三個功能或特色', pages: '101' }, { title: '圖 103、觀看串流影音平台具備最重要的三個功能或特色', pages: '102' }, { title: '圖 104、每週平均聆聽一個小時以上的Podcast 平台', pages: '103' }, { title: '圖 105、平均每週聆聽Podcast 的總時數', pages: '104' }, { title: '圖 106、在聆聽Podcast 時，必須具備哪三個最重要的功能或特色(複選)', pages: '104' }, { title: '圖 107、訂閱的Podcast 節目數', pages: '105' }, { title: '圖 108、通常在哪種情況下聆聽Podcast', pages: '106' }, { title: '圖 109、聆聽特定Podcast 節目原因', pages: '106' }, { title: '圖 110、發現新Podcast 的地方', pages: '107' }, { title: '圖 111、您最常聆聽哪類型的Podcast', pages: '108' }, { title: '圖 112、在最常聆聽的Podcast 類型，每週聆聽時數', pages: '108' }, { title: '圖 113、對Podcast 喜愛程度感受', pages: '109' }, { title: '圖 114、對Podcast 的感知價值 ', pages: '110' }, { title: '圖 115、對Podcast 的知覺易用性', pages: '111' }, { title: '圖 116、Podcast 的知覺有用性', pages: '112' }, { title: '圖 117、Podcast 的行為意圖', pages: '113' }, { title: '圖 118、最容易上手的Podcast', pages: '114' }, { title: '圖 119、搜尋內容最容易的Podcast', pages: '115' }, { title: '圖 120、最讓人信賴的Podcast', pages: '116' }, { title: '圖 121、內容最完整的Podcast', pages: '117' }, { title: '圖 122、最令人滿意度的Podcast', pages: '118' }, { title: '圖 123、比較容易找到要聽的Podcast 內容', pages: '119' }, { title: '圖 124、Z 世代對於選擇Podcast 的想法', pages: '120' }, { title: '圖 125、Z 世代預計要花多少分鐘才能找到要聆聽的內容', pages: '121' }, { title: '圖 126、願意聽具有置入性廣告的Podcast', pages: '122' }, { title: '圖 127、今年一月到三月曾經購買Podcast 上廣告所推銷的商品', pages: '123' }, { title: '圖 128、聽完Podcast 廣告後，會採取何種反應及行動', pages: '124' }, { title: '圖 129、未來訂閱取向-沒有意願聆聽Podcast 的原因', pages: '125' }, { title: '圖 130、未來有意願聆聽Podcast 的類型', pages: '125' }, { title: '圖 131、沒有意願聆聽Podcast 的原因', pages: '126' }, { title: '圖 128、聽完Podcast廣告後，會採取何種反應及行動', pages: '124' }, { title: '圖 129、未來訂閱取向-沒有意願聆聽Podcast的原因 ', pages: '125' }, { title: '圖 130、未來有意願聆聽Podcast的類型', pages: '125' }, { title: '圖 131、沒有意願聆聽Podcast的原因 ', pages: '126' }],
  tablecatalogue: [{ title: '表 1-1 串流影音的測量變項的建構信度及收斂效度', pages: '14' }, { title: '表 1-2 Podcast 的測量變項的建構信度及收斂效度分析', pages: '14' }],
  seoshare: {
    title: 'UUPON 串流影音及Podcast使用與消費行為調查',
    description: '2021市場調查研究：分別探討千禧世代與Z世代族群，分別在使用串流影音、Podcast上使用行為、偏好的異同。'
  }
}];

/* harmony default export */ var AllData = (dataList);
// CONCATENATED MODULE: ./node_modules/babel-loader/lib!./node_modules/vue-loader/lib/selector.js?type=script&index=0!./src/views/Home.vue
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//


/* harmony default export */ var Home = ({
  name: 'home',
  data: function data() {
    return {
      dataList: [],
      bannerList: [],
      swiperOptions: {
        autoplay: true,
        loop: true,
        spaceBetween: 15,
        pagination: {
          el: '.swiper-pagination',
          clickable: true
        }
      }
    };
  },
  created: function created() {
    this.dataList = AllData;
    for (var i = 0; i < this.dataList.length; i++) {
      if (this.dataList[i].isHot) {
        this.bannerList.push({ id: i, banner: this.dataList[i].banner, abstract: this.dataList[i].abstract, title: this.dataList[i].title });
      }
    }
    // document.querySelector('title').innerText = 'UUPON x 中大影響力管理與衡量研究中心'
    // document.querySelector('head meta[name="description"]').content = '2021市場調查研究： 串流影音及Podcast使用與消費行為調查、永續美妝保養品消費調查'
    // document.querySelector('head meta[property="og:title"]').content = 'UUPON x 中大影響力管理與衡量研究中心'
    // document.querySelector('head meta[property="og:description"]').content = '2021市場調查研究： 串流影音及Podcast使用與消費行為調查、永續美妝保養品消費調查'
  },

  computed: {},
  methods: {
    gotoIntroduce: function gotoIntroduce(item, index) {
      console.log(index, item.id, item.title);
      if (item.id) {
        // eslint-disable-next-line
        // gtag('event', 'Page Click', { 
        //   'event_category': 'Click',
        //   'event_label': `Banner_點擊${item.title}`,
        //   'value': 1
        // })
        console.log('Banner_\u9EDE\u64CA' + item.title);
        this.$router.push({ name: 'Introduce', query: { id: item.id } });
      } else {
        // eslint-disable-next-line
        // gtag('event', 'Page Click', { 
        //   'event_category': 'Click',
        //   'event_label': `最新消息_點擊${item.title}`,
        //   'value': 1
        // })
        console.log('\u6700\u65B0\u6D88\u606F_\u9EDE\u64CA' + item.title);
        this.$router.push({ name: 'Introduce', query: { id: index } });
      }
    }
  }

});
// CONCATENATED MODULE: ./node_modules/vue-loader/lib/template-compiler?{"id":"data-v-163fd0f5","hasScoped":false,"transformToRequire":{"video":["src","poster"],"source":"src","img":"src","image":"xlink:href"},"buble":{"transforms":{}}}!./node_modules/vue-loader/lib/selector.js?type=template&index=0!./src/views/Home.vue
var Home_render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{staticClass:"home mx-auto pb-5"},[_c('div',{staticClass:"container p-2"},[_c('div',{staticClass:"row"},[_c('div',{staticClass:"col"},[_c('router-link',{attrs:{"to":"/"}},[_c('img',{staticClass:"d-none d-md-block",attrs:{"src":__webpack_require__("dLd/"),"width":"235px","alt":""}})]),_vm._v(" "),_c('router-link',{attrs:{"to":"/"}},[_c('img',{staticClass:"d-md-none",attrs:{"src":__webpack_require__("dLd/"),"width":"135px","alt":""}})])],1)]),_vm._v(" "),_c('div',{staticClass:"row mt-2"},[_c('div',{staticClass:"col-12"},[_c('div',{staticClass:"banner"},[_c('swiper',{ref:"swiperComponent",attrs:{"options":_vm.swiperOptions}},[_vm._l((_vm.bannerList),function(item,index){return _c('swiper-slide',{key:index},[_c('div',{staticClass:"w-100 d-md-flex  banner-box p-1"},[_c('img',{staticClass:"d-md-block d-none cursor-pointer",attrs:{"src":item.banner,"width":"75%","alt":""},on:{"click":function($event){return _vm.gotoIntroduce(item, index)}}}),_vm._v(" "),_c('img',{staticClass:"d-md-none cursor-pointer",attrs:{"src":item.banner,"width":"100%","alt":""},on:{"click":function($event){return _vm.gotoIntroduce(item, index)}}}),_vm._v(" "),_c('div',{staticClass:"pl-3 pr-2 pt-3 d-none d-md-block w-25 banner-card"},[_c('p',{staticClass:"font-weight-bold h4 text-orange cursor-pointer word-break",on:{"click":function($event){return _vm.gotoIntroduce(item, index)}}},[_vm._v(_vm._s(item.title))]),_vm._v(" "),_c('p',{staticClass:"word-break h6",domProps:{"innerHTML":_vm._s(item.abstract.substring(0,35) + '...')}}),_vm._v(" "),_c('p',{staticClass:"cursor-pointer text-gray h6",on:{"click":function($event){return _vm.gotoIntroduce(item, index)}}},[_c('u',[_vm._v("了解更多")])])])])])}),_vm._v(" "),_c('div',{staticClass:"swiper-pagination",attrs:{"slot":"pagination"},slot:"pagination"})],2)],1)])]),_vm._v(" "),_vm._m(0),_vm._v(" "),_c('div',{staticClass:"row align-items-end"},_vm._l((_vm.dataList),function(item,index){return _c('div',{key:index,staticClass:"col-12 col-md-6 col-lg-4 mb-3"},[_c('p',{staticClass:"h4 text-orange"},[_vm._v(_vm._s(_vm.dataList[index].title))]),_vm._v(" "),_c('div',[_c('img',{attrs:{"src":_vm.dataList[index].img,"width":"100%","alt":""}}),_vm._v(" "),_c('p',{domProps:{"innerHTML":_vm._s(_vm.dataList[index].date)}}),_vm._v(" "),_c('p',{domProps:{"innerHTML":_vm._s(_vm.dataList[index].abstract.substring(0,30) + '...')}}),_vm._v(" "),_c('u',{staticClass:"text-gray cursor-pointer",on:{"click":function($event){return _vm.gotoIntroduce(item, index)}}},[_vm._v("了解更多")])])])}),0)])])}
var Home_staticRenderFns = [function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{staticClass:"row mt-5"},[_c('div',{staticClass:"col h1 font-weight-bold text-first"},[_vm._v("最新消息")])])}]
var Home_esExports = { render: Home_render, staticRenderFns: Home_staticRenderFns }
/* harmony default export */ var views_Home = (Home_esExports);
// CONCATENATED MODULE: ./src/views/Home.vue
function Home_injectStyle (ssrContext) {
  __webpack_require__("9WAJ")
}
var Home_normalizeComponent = __webpack_require__("VU/8")
/* script */


/* template */

/* template functional */
var Home___vue_template_functional__ = false
/* styles */
var Home___vue_styles__ = Home_injectStyle
/* scopeId */
var Home___vue_scopeId__ = null
/* moduleIdentifier (server only) */
var Home___vue_module_identifier__ = null
var Home_Component = Home_normalizeComponent(
  Home,
  views_Home,
  Home___vue_template_functional__,
  Home___vue_styles__,
  Home___vue_scopeId__,
  Home___vue_module_identifier__
)

/* harmony default export */ var src_views_Home = (Home_Component.exports);

// EXTERNAL MODULE: ./node_modules/babel-runtime/core-js/set.js
var set = __webpack_require__("lHA8");
var set_default = /*#__PURE__*/__webpack_require__.n(set);

// CONCATENATED MODULE: ./node_modules/babel-loader/lib!./node_modules/vue-loader/lib/selector.js?type=script&index=0!./src/views/Introduce.vue

//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//


/* harmony default export */ var Introduce = ({
  name: 'home',
  data: function data() {
    return {
      dataID: 0,
      tabVal: 'maincatalogue',
      data: [],
      moreArr: []
    };
  },
  created: function created() {
    this.init();
  },

  methods: {
    init: function init() {
      this.dataID = Number(this.$route.query.id);
      this.data = AllData;
      this.tabVal = 'maincatalogue';
      this.moreArr = [];
      // console.log(this.$route.query.id)
      function getRandom(min, max) {
        return Math.floor(Math.random() * (max - min + 1)) + min;
      }
      var mySet = new set_default.a();
      // eslint-disable-next-line
      var w = 0;
      if (this.data.length > 3) {
        do {
          w++;
          var j = getRandom(0, this.data.length - 1);
          if (j !== this.dataID) {
            mySet.add(j);
          }
        } while (mySet.size !== 3);
        this.moreArr = mySet;
        this.$nextTick(function () {
          window.scrollTo({
            top: 0,
            behavior: 'smooth'
          });
        });
      } else {
        for (var i = 0; i < this.data.length; i++) {
          if (i !== this.dataID) {
            this.moreArr.push(i);
          }
        }
      }
      // document.querySelector('title').innerText = this.data[this.dataID].seoshare.title
      // document.querySelector('head meta[name="description"]').content = this.data[this.dataID].seoshare.description
      // document.querySelector('head meta[property="og:title"]').content = this.data[this.dataID].seoshare.title
      // document.querySelector('head meta[property="og:description"]').content = this.data[this.dataID].seoshare.description
      console.log(document.querySelector('head meta[property="og:title"]'));
      window.scrollTo({
        top: 0,
        behavior: 'smooth'
      });
    },
    gotoSurvey: function gotoSurvey() {
      console.log('Google \u8868\u55AE\u8DF3\u8F49_' + this.data[this.dataID].title);
      // eslint-disable-next-line
      // gtag('event', 'Page Click', { 
      //   'event_category': 'Click',
      //   'event_label': `連結跳轉_${this.data[this.dataID].title}`,
      //   'value': 1
      // })
      window.open(this.data[this.dataID].url);
    },
    gotoIntroduce: function gotoIntroduce(i, title) {
      var _this = this;

      console.log(i, title, '\u66F4\u591A\u5831\u544A_\u9EDE\u64CA' + title);
      // eslint-disable-next-line
      // gtag('event', 'Page Click', { 
      //   'event_category': 'Click',
      //   'event_label': `更多報告點擊_${title}`,
      //   'value': 1
      // })
      this.$router.push({ name: 'Introduce', query: { id: i } });
      this.$nextTick(function () {
        _this.init();
      });
    }
  }
});
// CONCATENATED MODULE: ./node_modules/vue-loader/lib/template-compiler?{"id":"data-v-209ee597","hasScoped":false,"transformToRequire":{"video":["src","poster"],"source":"src","img":"src","image":"xlink:href"},"buble":{"transforms":{}}}!./node_modules/vue-loader/lib/selector.js?type=template&index=0!./src/views/Introduce.vue
var Introduce_render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{staticClass:"data pt-3 pb-5"},[_c('div',{staticClass:"container"},[_c('div',{staticClass:"row"},[_c('div',{staticClass:"col"},[_c('router-link',{attrs:{"to":"/"}},[_c('img',{staticClass:"d-none d-md-block",attrs:{"src":__webpack_require__("dLd/"),"width":"235px","alt":""}}),_vm._v(" "),_c('img',{staticClass:"d-md-none",attrs:{"src":__webpack_require__("dLd/"),"width":"135px","alt":""}})])],1)]),_vm._v(" "),_c('div',{staticClass:"row mt-2"},[_c('div',{staticClass:"col-12 col-md-8"},[_c('p',{staticClass:"font-weight-bold h4 text-orange"},[_vm._v(_vm._s(_vm.data[_vm.dataID].title))]),_vm._v(" "),_c('div',{staticClass:"text-gray mt-4"},[_c('img',{staticClass:"float-left mr-2",attrs:{"src":_vm.data[_vm.dataID].img,"width":"180px","alt":""}}),_vm._v(" "),_c('p',{staticClass:"h6font-weight-bold text-first",domProps:{"innerHTML":_vm._s(_vm.data[_vm.dataID].abstract)}}),_vm._v(" "),_c('br'),_vm._v(" "),(_vm.data[_vm.dataID].include.length>=1)?_c('p',{staticClass:"h4 font-weight-bold text-first"},[_vm._v("what's included??")]):_vm._e(),_vm._v(" "),_c('ul',{staticClass:"ml-4"},_vm._l((_vm.data[_vm.dataID].include),function(item,index){return _c('li',{key:index},[_vm._v(_vm._s(item))])}),0)])]),_vm._v(" "),_c('div',{staticClass:"d-none d-md-block col-md-4 mt-md-0 mt-4"},[_c('div',{staticClass:"border border-gray"},[_c('p',{staticClass:"p-3 h4 border-bottom border-gray text-first font-weight-bold"},[_vm._v("報告資訊")]),_vm._v(" "),_c('p',{staticClass:"p-3 h6 border-bottom border-gray text-first"},[_vm._v(_vm._s(_vm.data[_vm.dataID].title))]),_vm._v(" "),_c('div',{staticClass:"p-3 border-bottom border-gray text-first"},[_c('p',{staticClass:"h6 mb-2"},[_vm._v("頁數: "+_vm._s(_vm.data[_vm.dataID].pagecount))]),_vm._v(" "),_c('p',{staticClass:"h6 mb-2"},[_vm._v("檔案類型: "+_vm._s(_vm.data[_vm.dataID].filetype))])]),_vm._v(" "),_c('div',{staticClass:"p-3 text-center bg-light-orange"},[_c('button',{staticClass:"order-btn py-2 px-3",on:{"click":function($event){return _vm.gotoSurvey()}}},[_vm._v("聯絡我們")])])])])]),_vm._v(" "),_c('div',{staticClass:"row mt-3"},[_c('div',{staticClass:"col"},[_c('div',{staticClass:"d-flex border-bottom border-gray"},[_c('div',{staticClass:"px-3 py-2 cursor-pointer",class:{'tabItem':_vm.tabVal === 'maincatalogue'},on:{"click":function($event){_vm.tabVal ='maincatalogue'}}},[_vm._v("章節目錄")]),_vm._v(" "),_c('div',{staticClass:"px-3 py-2 cursor-pointer",class:{'tabItem':_vm.tabVal === 'picturecatalogue'},on:{"click":function($event){_vm.tabVal = 'picturecatalogue'}}},[_vm._v("圖目錄")]),_vm._v(" "),_c('div',{staticClass:"px-3 py-2 cursor-pointer",class:{'tabItem':_vm.tabVal === 'tablecatalogue'},on:{"click":function($event){_vm.tabVal = 'tablecatalogue'}}},[_vm._v("表目錄")])])])]),_vm._v(" "),_c('div',{staticClass:"row mt-3"},[_c('div',{staticClass:"col"},[(_vm.tabVal === 'maincatalogue')?_c('div',_vm._l((_vm.data[_vm.dataID].maincatalogue),function(item,index){return _c('div',{key:index},[_c('div',{staticClass:"d-flex justify-content-between align-items-end mb-2 text-first"},[_c('span',{staticClass:"mr-2 text-nowrap"},[_vm._v(_vm._s(item.title))]),_vm._v(" "),_c('span',{staticClass:"float-right"},[_vm._v(_vm._s(item.pages))])]),_vm._v(" "),(item.content)?_c('div',_vm._l((item.content),function(i,index){return _c('div',{key:index,staticClass:"ml-3 d-flex justify-content-between align-items-end mb-2 text-first"},[_c('span',{staticClass:"mr-2 text-nowrap"},[_vm._v(_vm._s(i.title))]),_vm._v(" "),_c('span',{staticClass:"float-right"},[_vm._v(_vm._s(i.pages))])])}),0):_vm._e()])}),0):_vm._e(),_vm._v(" "),(_vm.tabVal === 'picturecatalogue')?_c('div',_vm._l((_vm.data[_vm.dataID].picturecatalogue),function(item,index){return _c('div',{key:index,staticClass:"d-flex justify-content-between align-items-end mb-2 text-first"},[_c('span',{staticClass:"mr-2 text-nowrap"},[_vm._v(_vm._s(item.title))]),_vm._v(" "),_c('div',{staticClass:" border-dotted w-100 mr-1"}),_vm._v(" "),_c('span',{},[_vm._v(_vm._s(item.pages))])])}),0):_vm._e(),_vm._v(" "),(_vm.tabVal === 'tablecatalogue')?_c('div',_vm._l((_vm.data[_vm.dataID].tablecatalogue),function(item,index){return _c('div',{key:index,staticClass:"d-flex justify-content-between align-items-end mb-2 text-first"},[_c('span',{staticClass:"mr-2 text-nowrap"},[_vm._v(_vm._s(item.title))]),_vm._v(" "),_c('div',{staticClass:" border-dotted w-100 mr-1"}),_vm._v(" "),_c('span',{staticClass:"float-right"},[_vm._v(_vm._s(item.pages))])])}),0):_vm._e()])]),_vm._v(" "),_c('div',{staticClass:"row mt-3 d-md-none"},[_c('div',{staticClass:"col-md-4 mt-md-0 mt-4"},[_c('div',{staticClass:"border border-gray"},[_c('p',{staticClass:"p-3 h4 border-bottom border-gray text-first font-weight-bold"},[_vm._v("報告資訊")]),_vm._v(" "),_c('p',{staticClass:"p-3 h6 border-bottom border-gray text-first"},[_vm._v(_vm._s(_vm.data[_vm.dataID].title))]),_vm._v(" "),_c('div',{staticClass:"p-3 border-bottom border-gray text-first"},[_c('p',{staticClass:"h6 mb-2"},[_vm._v("頁數: "+_vm._s(_vm.data[_vm.dataID].pagecount))]),_vm._v(" "),_c('p',{staticClass:"h6 mb-2"},[_vm._v("檔案類型: "+_vm._s(_vm.data[_vm.dataID].filetype))])]),_vm._v(" "),_c('div',{staticClass:"p-3 text-center bg-light-orange"},[_c('button',{staticClass:"order-btn py-2 px-3",on:{"click":function($event){return _vm.gotoSurvey()}}},[_vm._v("聯絡我們")])])])])]),_vm._v(" "),_c('div',{staticClass:"row mt-3 d-md-block d-none"},[_c('div',{staticClass:"col text-center"},[_c('button',{staticClass:"order-btn py-2 px-3",on:{"click":function($event){return _vm.gotoSurvey()}}},[_vm._v("聯絡我們")])])]),_vm._v(" "),_vm._m(0),_vm._v(" "),_c('div',{staticClass:"row mt-1"},_vm._l((_vm.moreArr),function(i,index){return _c('div',{key:index,staticClass:"col-12 col-md-6 col-lg-4 mb-3"},[_c('img',{staticClass:"cursor-pointer",attrs:{"src":_vm.data[i].img,"width":"100%","alt":""},on:{"click":function($event){return _vm.gotoIntroduce(i, _vm.data[i].title)}}}),_vm._v(" "),_c('p',{staticClass:"h4 text-orange cursor-pointer",on:{"click":function($event){return _vm.gotoIntroduce(i, _vm.data[i].title)}}},[_vm._v(_vm._s(_vm.data[i].title))])])}),0)])])}
var Introduce_staticRenderFns = [function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{staticClass:"row mt-4"},[_c('div',{staticClass:"col"},[_c('p',{staticClass:"h2 font-weight-bold"},[_vm._v("更多報告")])])])}]
var Introduce_esExports = { render: Introduce_render, staticRenderFns: Introduce_staticRenderFns }
/* harmony default export */ var views_Introduce = (Introduce_esExports);
// CONCATENATED MODULE: ./src/views/Introduce.vue
function Introduce_injectStyle (ssrContext) {
  __webpack_require__("PLJb")
}
var Introduce_normalizeComponent = __webpack_require__("VU/8")
/* script */


/* template */

/* template functional */
var Introduce___vue_template_functional__ = false
/* styles */
var Introduce___vue_styles__ = Introduce_injectStyle
/* scopeId */
var Introduce___vue_scopeId__ = null
/* moduleIdentifier (server only) */
var Introduce___vue_module_identifier__ = null
var Introduce_Component = Introduce_normalizeComponent(
  Introduce,
  views_Introduce,
  Introduce___vue_template_functional__,
  Introduce___vue_styles__,
  Introduce___vue_scopeId__,
  Introduce___vue_module_identifier__
)

/* harmony default export */ var src_views_Introduce = (Introduce_Component.exports);

// CONCATENATED MODULE: ./src/router/index.js







vue_esm["a" /* default */].use(vue_router_esm["a" /* default */]);
vue_esm["a" /* default */].use(vue_meta_esm["a" /* default */], {
  keyName: 'head'
});

var router = new vue_router_esm["a" /* default */]({
  mode: 'hash',
  routes: [{
    path: '/',
    name: 'Home',
    component: src_views_Home,
    meta: {
      requiresAuth: true,
      title: 'UUPON x 中大影響力管理與衡量研究中心',
      metaTags: [{
        name: 'description',
        content: '2021市場調查研究： 串流影音及Podcast使用與消費行為調查、永續美妝保養品消費調查'
      }, {
        property: 'og:description',
        content: '2021市場調查研究： 串流影音及Podcast使用與消費行為調查、永續美妝保養品消費調查'
      }]
    }
  }, {
    path: '/introduce',
    name: 'Introduce',
    component: src_views_Introduce,
    meta: {
      requiresAuth: true,
      title: 'UUPON 永續美妝保養品消費調查',
      metaTags: [{
        name: 'description',
        content: '2021市場調查研究：美妝保養品消費行為、環保永續美妝保養品牌評價、永續美妝保養品願意支付金額'
      }, {
        property: 'og:description',
        content: '2021市場調查研究：美妝保養品消費行為、環保永續美妝保養品牌評價、永續美妝保養品願意支付金額'
      }]
    }
  }]
});
// router.beforeEach((to, from, next) => {
//   window.document.title = to.meta.title
//   next()
// })
// ...

// This callback runs before every route change, including on page load.
router.beforeEach(function (to, from, next) {
  // This goes through the matched routes from last to first, finding the closest route with a title.
  // e.g., if we have `/some/deep/nested/route` and `/some`, `/deep`, and `/nested` have titles,
  // `/nested`'s will be chosen.
  var nearestWithTitle = to.matched.slice().reverse().find(function (r) {
    return r.meta && r.meta.title;
  });

  // Find the nearest route element with meta tags.
  var nearestWithMeta = to.matched.slice().reverse().find(function (r) {
    return r.meta && r.meta.metaTags;
  });

  var previousNearestWithMeta = from.matched.slice().reverse().find(function (r) {
    return r.meta && r.meta.metaTags;
  });

  // If a route with a title was found, set the document (page) title to that value.
  if (nearestWithTitle) {
    document.title = nearestWithTitle.meta.title;
  } else if (previousNearestWithMeta) {
    document.title = previousNearestWithMeta.meta.title;
  }

  // Remove any stale meta tags from the document using the key attribute we set below.
  from_default()(document.querySelectorAll('[data-vue-router-controlled]')).map(function (el) {
    return el.parentNode.removeChild(el);
  });

  // Skip rendering meta tags if there are none.
  if (!nearestWithMeta) return next();

  // Turn the meta tag definitions into actual elements in the head.
  nearestWithMeta.meta.metaTags.map(function (tagDef) {
    var tag = document.createElement('meta');

    keys_default()(tagDef).forEach(function (key) {
      tag.setAttribute(key, tagDef[key]);
    });

    // We use this to track which meta tags we create so we don't interfere with other ones.
    tag.setAttribute('data-vue-router-controlled', '');

    return tag;
  }).forEach(function (tag) {
    return document.head.appendChild(tag);
  });

  // Add the meta tags to the document head.
  next();
});
/* harmony default export */ var src_router = (router);
// EXTERNAL MODULE: ./src/assets/all.scss
var assets_all = __webpack_require__("pc4J");
var all_default = /*#__PURE__*/__webpack_require__.n(assets_all);

// EXTERNAL MODULE: ./node_modules/axios/index.js
var axios = __webpack_require__("mtWM");
var axios_default = /*#__PURE__*/__webpack_require__.n(axios);

// EXTERNAL MODULE: ./node_modules/vue-awesome-swiper/dist/vue-awesome-swiper.js
var vue_awesome_swiper = __webpack_require__("7QTg");
var vue_awesome_swiper_default = /*#__PURE__*/__webpack_require__.n(vue_awesome_swiper);

// EXTERNAL MODULE: ./node_modules/swiper/swiper-bundle.css
var swiper_bundle = __webpack_require__("wNQb");
var swiper_bundle_default = /*#__PURE__*/__webpack_require__.n(swiper_bundle);

// CONCATENATED MODULE: ./src/main.js
// The Vue build version to load with the `import` command
// (runtime-only or standalone) has been set in webpack.base.conf with an alias.






// import './assets/js/loader.min.js'



vue_esm["a" /* default */].use(vue_awesome_swiper_default.a);
vue_esm["a" /* default */].prototype.$http = axios_default.a;
vue_esm["a" /* default */].config.productionTip = false;

/* eslint-disable no-new */
new vue_esm["a" /* default */]({
  el: '#app',
  router: src_router,
  components: { App: src_App },
  template: '<App/>'
});

/***/ }),

/***/ "PLJb":
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ "WCFv":
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ "dLd/":
/***/ (function(module, exports) {

module.exports = "data:image/png;base64,"

/***/ }),

/***/ "j/ML":
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__.p + "static/img/picture_3_makep.95ef576.jpeg";

/***/ }),

/***/ "maJG":
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__.p + "static/img/banner_5_podcast.d0d7ff2.jpeg";

/***/ }),

/***/ "nbZn":
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__.p + "static/img/banner_3_makep.60b1e9d.jpeg";

/***/ }),

/***/ "pc4J":
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ }),

/***/ "wNQb":
/***/ (function(module, exports) {

// removed by extract-text-webpack-plugin

/***/ })

},["NHnr"]);
//# sourceMappingURL=app-app20210917-a.js.map